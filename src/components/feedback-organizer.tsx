...
export function FeedbackOrganizer() {
  ...
  const [currentView, setCurrentView] = useState<"incoming" | "organized">("incoming");
  ...
  const mockFeedback: FeedbackItem[] = [
    {
      ...
      description: "Would love to have a dark theme option for better night usage",
      sentiment: "positive",
      priority: "high",
      votes: 127,
      status: "new",
    ...
    },
    {
      ...
      description: "Users are getting logged out too frequently, very frustrating",
      sentiment: "negative",
      priority: "high",
      votes: 89,
      status: "new",
    ...
    },
    {
      ...
      description: "Please create a mobile app version, web app is not responsive enough",
      sentiment: "neutral",
      priority: "medium",
      votes: 156,
      status: "new",
  ...
  const [categoryStats, setCategoryStats] = useState<Record<string, CategoryStats>>({
    feature: { total: 0, positive: 0, neutral: 0, negative: 0, trending: false },
    bug: { total: 0, positive: 0, neutral: 0, negative: 0, trending: false },
    general: { total: 0, positive: 0, neutral: 0, negative: 0, trending: false }
  });
      ...
      const addNextItem = (index: number) => {
        if (index < mockFeedback.length) {
          ...
          setTimeout(() => {
            ...
            // Calculate stats
            const stats = mockFeedback.reduce((acc, item) => {
              if (!acc[item.type]) {
                acc[item.type] = { total: 0, positive: 0, neutral: 0, negative: 0, trending: false };
              }
              acc[item.type].total++;
  ...
  const getStatusIcon = (status: FeedbackItem["status"]) => {
    switch (status) {
      ...
      case "planned": return <AlertCircle className="w-3 h-3 text-yellow-600" />;
      default: return <div className="w-3 h-3 rounded-full bg-neutral-400" />;
    }
  };

  return (
    <div ref={ref} className="w-full max-w-2xl mx-auto bg-white border border-neutral-200 rounded-lg overflow-hidden">
      {/* Header */}
          ...
          {currentView === "incoming" ? (
            ...
            >
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-sm font-medium text-neutral-700">Incoming Feedback</span>
              </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        <span className="text-xs text-neutral-500">{item.votes} votes</span>
                      </div>
                    </div>
                  </div>
                </motion.div>
            ...
            >
              ...
              <div className="space-y-3">
                <h4 className="text-sm font-medium text-neutral-700">Prioritized Items</h4>
                
                {["high", "medium", "low"].map((priority) => {
                  const items = feedbackItems.filter(item => item.priority === priority);
                        ...
                        >
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <div className="flex items-center gap-1 ml-auto">
                              ...
                              <span className="text-neutral-600">{item.votes}</span>
                            </div>
                          </div>
                          <div className="flex gap-1">
                            {item.tags.map((tag) => (
                              <span key={tag} className="px-1 py-0.5 bg-neutral-200 text-neutral-600 rounded text-xs">
                                {tag}
                              </span>
                            ))}
                          </div>
