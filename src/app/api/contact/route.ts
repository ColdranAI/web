...
async function sendWebhook(data: ContactFormData, metadata: { ip: string; userAgent: string; timestamp: string }) {
  ...
  try {
    ...
    return NextResponse.json(
      ...
      { 
        ...
        headers: {
          'X-RateLimit-Limit': MAX_REQUESTS_PER_WINDOW.toString(),
          'X-RateLimit-Remaining': (MAX_REQUESTS_PER_WINDOW - (RATE_LIMIT_STORAGE.get(clientIP)?.count || 0)).toString()
        }
      }
    );
